...
const PDFBlock = ({ item, index, expandedItem, setExpandedItem }) => {
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          {item.status === "clean" ? <ShieldCheck className="text-green-400 mr-2" size={20} /> : <AlertCircle className="text-red-400 mr-2" size={20} />}
      ...
      <div className="flex justify-end mt-2">
        <ChevronDown size={20} className={`transition-transform duration-300 ${expandedItem === index ? "rotate-180" : "rotate-0"} text-gray-400`} />
      </div>

      {expandedItem === index && item.details !== "No threats found" && (
        <motion.div
        ...
        >
          ...
          <ul className="list-disc list-inside mt-2 space-y-1">
            {item.yara_details?.map((detail, i) => (
              <li key={i} className="text-sm text-gray-400 border-b border-gray-700 pb-2">
                <p className="text-gray-300 font-semibold">{detail.rule}</p>
                <p className="text-gray-400">{detail.description}</p>
                <p className="text-gray-400 text-xs">Weight: {detail.weight}</p>
              </li>
            ))}
          </ul>
        </motion.div>
